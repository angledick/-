"""
SSE流式对话（Cowork层入口）+ 主动引擎 + Agent管理API。

全功能SSE对话管线:
  POST /api/v1/chat/stream          — SSE流式对话（主入口）
...
Agent管理:
  ...
  GET  /api/v1/agents/tasks/{id}    — 任务进度
  POST /api/v1/agents/tasks/{id}/intervene — 用户干预
  GET  /api/v1/agents/workers       — Worker状态
  GET  /api/v1/agents/templates     — 任务分解模板

主动引擎:
  GET  /api/v1/proactive/heartbeat  — 心跳状态
...
开源参考:
  - Open WebUI (139k⭐): SSE流式对话模式
  - Chatwoot (29.9k⭐): 全渠道消息处理
  - Temporal (14k⭐): 确定性工作流执行
"""
