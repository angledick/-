...
S["store.py<br/>知识存储"]
CR["core/rag.py<br/>核心RAG"]
AR["api/rag.py<br/>API RAG"]
...
- 知识存储（store.py）：封装向量数据库（Chroma）与本地JSON存储，提供索引构建、写入、删除与查询接口；维护全局知识与产品级知识目录。
- 核心RAG（core/rag.py）：统一的检索增强生成引擎，包含向量化、相似度检索、上下文拼接、提示工程与生成控制。
- API RAG（api/rag.py）：对外暴露的REST接口，接收用户问题、调用RAG引擎并返回结果。
...
- 向量化模型
  ...
  - 向量维度与模型选择由配置文件与RAG引擎共同决定。
- 相似度计算
  - 采用向量空间相似度（如余弦相似度）在Chroma中进行检索。
...
participant R as "RAG引擎(core/rag.py)"
participant P as "提示词加载(services/prompt_loader.py)"
participant M as "模型路由(config/model_routes.json)"
Q->>R : "问题+上下文需求"
R->>P : "加载场景提示词"
P-->>R : "模板+变量"
R->>M : "选择模型与策略"
...
## 依赖关系分析
- 组件耦合
  - API层依赖核心RAG；核心RAG依赖存储与提示词加载；存储依赖向量数据库与本地JSON目录。
- 外部依赖
  - Chroma向量数据库、YAML提示词模板、技能与模型路由配置。
- 可能的循环依赖
  - 当前模块间通过清晰职责边界避免循环依赖：API→RAG→Store→VectorDB；PromptLoader独立注入。
...
CORE --> PROMPT["services/prompt_loader.py"]
CORE --> CONFIG1["config/skills/registry.json"]
CORE --> CONFIG2["config/model_routes.json"]
```
...
## 结论
本知识管理系统以“加载—存储—检索—生成”为主线，结合市场路由与提示工程，实现了可扩展、可维护的知识服务。通过向量检索与结构化存储的协同，系统在保证召回质量的同时兼顾性能；通过配置化的提示词与模型路由，确保输出的一致性与合规性。建议在生产环境中持续优化阈值与top-k、完善元数据标注，并建立完善的版本与回滚机制。
