...
- [skills\registry.json](file://backend/data/config/skills/registry.json)
...
- 配置与技能资产：位于 backend/data/config/skills/registry.json 作为技能注册表持久化；.agents/skills 下存放各技能的 SKILL.md 描述文件。
...
subgraph "配置与资产"
CFG["skills/registry.json<br/>注册表持久化"]
...
## 核心组件
- 技能注册表（SkillRegistry）：负责技能的注册、安装、卸载、启用/禁用、查询、配置更新、状态与统计维护，并持久化到 registry.json。
...
participant Store as "持久化存储(skills/registry.json)"
Task->>SDK : 尝试SDK执行
SDK-->>Task : 失败或异常
...
- 持久化：将注册表写入 skills/registry.json，确保重启后状态一致。
...
- 外部依赖
  ...
  - 持久化：skills/registry.json 存储注册表，保障状态一致性。
...
- 持久化频率：注册表变更按需持久化，减少磁盘 IO；批量更新时注意事务性。
...
## 结论
避风港平台的技能管理系统以 SkillRegistry 为核心，配合 SkillExecutor 与 SkillRecommender 实现了从技能注册、配置、执行到推荐的完整闭环。通过阶段映射矩阵与跨阶段通用技能，系统在不同业务阶段提供一致的能力支撑；通过统计与安全扫描，保障执行质量与合规性。结合 ManagerAgent 的回退机制，系统在复杂场景下仍能稳定运行。
...
- 部署流程
  ...
  - 持久化至 skills/registry.json，重启后生效。
