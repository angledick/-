# 技能管理系统

<cite>
**本文引用的文件**
- [skill_registry.py](file://backend/app/core/skill_registry.py)
- [manager_agent.py](file://backend/app/core/manager_agent.py)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)
- [SKILL.md 示例](file://.agents/skills/gpt-taste/SKILL.md)
- [shopify_reference.md](file://backend/data/context/shopify_reference.md)
- [后端api.md](file://后端api.md)
- [前后端api交互.md](file://前后端api交互.md)
</cite>

## 更新摘要
**所做更改**
- 更新了技能推荐系统架构，从内部_yaml配置文件迁移到外部context参考文档
- 移除了_stage_matrix.yaml和_event_actions.yaml配置文件，技能推荐逻辑现在通过data/context/目录下的markdown文档实现
- 技能推荐器接口保持兼容性但返回空列表，实际推荐逻辑由agent运行时直接读取context文档完成
- 更新了技能注册表的持久化机制和配置管理方式

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本文件面向避风港平台的技能管理系统，围绕技能注册表、技能执行器与技能推荐器进行系统性技术文档化，覆盖技能元数据管理、版本控制、依赖关系、执行流程、推荐策略、配置管理、扩展机制以及开发与集成实践。文档以代码为依据，结合架构图与流程图帮助读者快速理解并高效使用该系统。

**更新** 技能推荐系统已从内部_yaml配置文件重构为外部context参考文档模式，技能信息已迁移到data/context/目录下的markdown文档中，推荐逻辑由agent运行时直接读取这些文档完成。

## 项目结构
技能系统主要由三部分构成：
- 核心注册与执行：位于 backend/app/core/skill_registry.py，包含技能注册表、技能执行器、技能推荐器及阶段映射矩阵等。
- 业务编排接入：位于 backend/app/core/manager_agent.py，负责在任务执行中回退到技能执行器。
- 配置与技能资产：位于 backend/data/skills/_registry.yaml 作为新的技能注册表配置；backend/data/skills/registry.json 作为兼容性保留；backend/data/context/ 目录下存放外部context参考文档；.agents/skills 下存放各技能的 SKILL.md 描述文件。

```mermaid
graph TB
subgraph "后端核心"
SR["SkillRegistry<br/>技能注册表"]
SE["SkillExecutor<br/>技能执行器"]
RE["SkillRecommender<br/>技能推荐器"]
MM["ManagerAgent<br/>任务编排"]
end
subgraph "配置与资产"
CFG["skills/_registry.yaml<br/>新式配置文件"]
JSON["skills/registry.json<br/>兼容性保留"]
CTX["data/context/*.md<br/>外部context参考文档"]
ASSET[".agents/skills/*<br/>SKILL.md 描述文件"]
end
MM --> SE
SE --> SR
RE --> SR
SR <- --> CFG
SR <- --> JSON
SR <- --> CTX
SR <- --> ASSET
```

**图表来源**
- [skill_registry.py:1-966](file://backend/app/core/skill_registry.py#L1-L966)
- [manager_agent.py:509-543](file://backend/app/core/manager_agent.py#L509-L543)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)
- [shopify_reference.md](file://backend/data/context/shopify_reference.md)
- [SKILL.md 示例](file://.agents/skills/gpt-taste/SKILL.md)

**章节来源**
- [skill_registry.py:1-966](file://backend/app/core/skill_registry.py#L1-L966)
- [manager_agent.py:509-543](file://backend/app/core/manager_agent.py#L509-L543)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)
- [shopify_reference.md](file://backend/data/context/shopify_reference.md)
- [SKILL.md 示例](file://.agents/skills/gpt-taste/SKILL.md)

## 核心组件
- 技能注册表（SkillRegistry）：负责技能的注册、安装、卸载、启用/禁用、查询、配置更新、状态与统计维护，并支持 YAML 和 JSON 两种配置格式的持久化。
- 技能执行器（SkillExecutor）：负责加载技能、传递参数、执行并处理结果，支持超时控制与执行统计更新。
- **更新** 技能推荐器（SkillRecommender）：现已迁移到外部context参考文档模式，接口保持兼容性但返回空列表，实际推荐逻辑由agent运行时直接读取data/context/目录下的markdown文档完成。
- 任务编排接入：ManagerAgent 在 SDK 执行失败时回退到技能执行器，统一对外输出标准化结果。

**章节来源**
- [skill_registry.py:1-966](file://backend/app/core/skill_registry.py#L1-L966)
- [manager_agent.py:509-543](file://backend/app/core/manager_agent.py#L509-L543)

## 架构总览
下图展示从任务编排到技能执行的整体流程，以及技能注册表与外部资产的交互。

```mermaid
sequenceDiagram
participant Task as "任务编排(ManagerAgent)"
participant SDK as "SDK执行(Worker)"
participant Exec as "技能执行器(SkillExecutor)"
participant Reg as "技能注册表(SkillRegistry)"
participant Yaml as "YAML配置(skills/_registry.yaml)"
participant Json as "JSON配置(skills/registry.json)"
participant Ctx as "外部context文档(data/context/*.md)"
Task->>SDK : 尝试SDK执行
SDK-->>Task : 失败或异常
Task->>Exec : 回退执行(skill_name, args, timeout)
Exec->>Reg : 查询技能信息/依赖/权限
Reg-->>Exec : 返回技能元数据
Reg->>Yaml : 读取新式配置
Reg->>Json : 读取兼容性配置
Reg->>Ctx : 读取外部context参考文档
Exec->>Exec : 加载并执行技能
Exec-->>Task : 返回执行结果/错误
Exec->>Reg : 更新统计(成功/失败/耗时)
Reg->>Yaml : 持久化新式配置
Reg->>Json : 持久化兼容性配置
```

**图表来源**
- [manager_agent.py:521-535](file://backend/app/core/manager_agent.py#L521-L535)
- [skill_registry.py:410-476](file://backend/app/core/skill_registry.py#L410-L476)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)
- [shopify_reference.md](file://backend/data/context/shopify_reference.md)

## 详细组件分析

### 技能注册表（SkillRegistry）
职责与能力
- 元数据管理：维护技能的标识、名称、描述、版本、作者、来源、文件路径、状态、适用阶段、事件类型、权限、依赖、配置、统计指标等。
- 生命周期管理：安装、卸载、启用、禁用、查询、配置更新。
- 统计与追踪：记录安装次数、成功率、平均耗时、最后使用时间等。
- **更新** 多格式持久化：支持 YAML 和 JSON 两种配置格式的读写，确保向后兼容性。

关键数据结构
- SkillInfo：技能元数据与统计字段的数据类。
- SkillExecution：执行记录，包含状态、参数、结果、错误、起止时间与耗时。

实现要点
- 版本控制：通过 version 字段与 source/source_url 表征来源与版本，便于升级与回滚。
- 依赖关系：dependencies 字段声明前置技能，执行前可按需校验或排序。
- 权限与安全：required_permissions 字段用于安全扫描与授权控制；security_scan 记录扫描结果。
- **更新** 阶段映射：SKILLS_STAGE_MATRIX 提供阶段到技能的映射，CROSS_STAGE_SKILLS 提供跨阶段通用技能，现已迁移到外部context参考文档。
- **更新** 配置兼容性：同时支持 _registry.yaml 和 registry.json 两种格式，新系统优先使用 YAML 格式。

```mermaid
classDiagram
class SkillInfo {
+string id
+string name
+string version
+string source
+string file_path
+SkillStatus status
+string[] business_stages
+string[] event_types
+string[] required_permissions
+string[] dependencies
+Dict config
+int install_count
+float success_rate
+int avg_execution_ms
+string last_used_at
+to_dict()
+from_dict(data)
}
class SkillExecution {
+string id
+string skill_id
+string skill_name
+string status
+Dict args
+Any result
+string error
+float started_at
+float finished_at
+int duration_ms
+to_dict()
}
class SkillRegistry {
+register_skill(info)
+install_skill(skill_id)
+uninstall_skill(skill_id)
+enable_skill(skill_id)
+disable_skill(skill_id)
+update_config(skill_id, config)
+get_by_id(skill_id)
+list_skills()
+get_stage_matrix()
+get_cross_stage_skills()
+_persist()
+_load_yaml_config()
+_load_json_config()
}
SkillRegistry --> SkillInfo : "管理"
SkillRegistry --> SkillExecution : "记录执行"
```

**图表来源**
- [skill_registry.py:33-96](file://backend/app/core/skill_registry.py#L33-L96)

**章节来源**
- [skill_registry.py:33-408](file://backend/app/core/skill_registry.py#L33-L408)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)

### 技能执行器（SkillExecutor）
职责与能力
- 技能加载：根据技能名定位 SKILL.md 或本地资源，解析执行入口与参数。
- 参数传递：接收任务上下文参数，注入 task_id、action 等关键字段。
- 执行与结果处理：异步执行技能，捕获超时、异常，返回标准化结果。
- 统计更新：根据执行结果更新技能的成功率、平均耗时与最后使用时间。

执行流程
```mermaid
flowchart TD
Start(["进入 execute"]) --> BuildArgs["构建执行参数<br/>合并上下文+task_id+action"]
BuildArgs --> RecordExec["记录执行记录<br/>pending->running"]
RecordExec --> RunSkill["调用 _run_skill 执行"]
RunSkill --> Success{"执行成功？"}
Success --> |是| UpdateStats["更新技能统计<br/>成功+耗时"]
Success --> |否| HandleErr["处理异常/超时"]
UpdateStats --> ReturnOK["返回成功结果"]
HandleErr --> ReturnErr["返回错误/超时"]
```

**图表来源**
- [skill_registry.py:442-476](file://backend/app/core/skill_registry.py#L442-L476)
- [skill_registry.py:855-866](file://backend/app/core/skill_registry.py#L855-L866)

**章节来源**
- [skill_registry.py:410-476](file://backend/app/core/skill_registry.py#L410-L476)
- [skill_registry.py:855-866](file://backend/app/core/skill_registry.py#L855-L866)

### **更新** 技能推荐器（SkillRecommender）
职责与能力
- **更新** 推荐逻辑迁移：技能信息已迁移到 data/context/ 目录下的 agent 参考文档，推荐逻辑由 agent 运行时直接读取这些文档完成。
- 接口兼容性：为保持向后兼容，推荐器接口仍然存在但返回空列表。
- **更新** 外部context集成：推荐系统现在依赖外部 markdown 文档提供的技能信息和上下文规则。

推荐策略
- **更新** 文档驱动：技能推荐不再依赖内部_yaml配置，而是通过读取 data/context/ 目录下的 markdown 文档实现。
- **更新** 动态上下文：推荐逻辑根据业务阶段、事件类型、产品类型等上下文动态生成推荐结果。
- **更新** agent运行时：实际的推荐决策在 agent 运行时环境中完成，技能推荐器仅提供接口兼容性。

**章节来源**
- [skill_registry.py:688-708](file://backend/app/core/skill_registry.py#L688-L708)
- [shopify_reference.md:1-112](file://backend/data/context/shopify_reference.md#L1-L112)

### 任务编排回退机制（ManagerAgent）
当 SDK 执行失败时，ManagerAgent 自动回退到技能执行器，统一输出结构包含 worker、task_type、sdk_executed 标记、状态与错误信息，确保上层调用的一致性。

**章节来源**
- [manager_agent.py:515-543](file://backend/app/core/manager_agent.py#L515-L543)

## 依赖分析
- 内部耦合
  - SkillExecutor 依赖 SkillRegistry 进行技能查询与统计更新。
  - **更新** SkillRecommender 现已解耦，不再依赖内部配置，改为读取外部context文档。
  - ManagerAgent 依赖 SkillExecutor 进行回退执行。
- 外部依赖
  - 技能资产：.agents/skills 下的 SKILL.md 描述文件，提供技能元数据与执行入口。
  - **更新** 配置文件：skills/_registry.yaml 提供新式配置，skills/registry.json 保持兼容性。
  - **更新** 外部context文档：backend/data/context/ 目录下的 markdown 文档提供技能推荐所需的上下文信息。

```mermaid
graph LR
MA["ManagerAgent"] --> SE["SkillExecutor"]
SE --> SR["SkillRegistry"]
RE["SkillRecommender"] --> SR
SR --> YAML["skills/_registry.yaml"]
SR --> JSON["skills/registry.json"]
SR --> CTX["data/context/*.md"]
SR --> ASSET[".agents/skills/*"]
```

**图表来源**
- [manager_agent.py:521-535](file://backend/app/core/manager_agent.py#L521-L535)
- [skill_registry.py:949-966](file://backend/app/core/skill_registry.py#L949-L966)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)
- [shopify_reference.md](file://backend/data/context/shopify_reference.md)
- [.agents/skills](file://.agents/skills)

**章节来源**
- [manager_agent.py:509-543](file://backend/app/core/manager_agent.py#L509-L543)
- [skill_registry.py:949-966](file://backend/app/core/skill_registry.py#L949-L966)

## 性能考虑
- 超时控制：执行器默认超时可配置，避免长时间阻塞；超时即刻返回，降低任务堆积风险。
- 统计驱动优化：通过成功率与平均耗时指导技能选择与重试策略，减少失败开销。
- 并发与隔离：执行器内部采用异步执行与独立的执行记录，避免相互干扰。
- **更新** 配置读取优化：新式 YAML 配置文件具有更好的解析性能，同时保持 JSON 兼容性以确保平滑迁移。
- **更新** 外部文档缓存：外部context文档的读取可以进行缓存优化，减少重复解析开销。

## 故障排查指南
常见问题与定位
- 技能未找到或加载失败
  - 检查技能是否已安装且状态为 active。
  - 核对 file_path 与 SKILL.md 是否存在。
  - 查看执行记录中的错误信息与耗时。
- 权限不足或安全扫描失败
  - 检查 required_permissions 与 security_scan 结果。
  - 确认执行环境具备所需权限。
- 超时或执行异常
  - 调整执行器超时阈值。
  - 分析执行统计，优先选择成功率高、耗时短的技能。
- **更新** 推荐不准确
  - 检查外部context文档是否正确配置。
  - 确认业务阶段与事件类型是否正确传入。
  - 验证 data/context/ 目录下的 markdown 文档格式。
- **更新** 配置文件问题
  - 确认 YAML 配置文件格式正确，优先使用 _registry.yaml。
  - 如遇兼容性问题，检查 registry.json 是否存在且格式有效。
  - **更新** 检查外部context文档是否存在且格式正确。

**章节来源**
- [skill_registry.py:467-476](file://backend/app/core/skill_registry.py#L467-L476)
- [skill_registry.py:855-866](file://backend/app/core/skill_registry.py#L855-L866)

## 结论
避风港平台的技能管理系统以 SkillRegistry 为核心，配合 SkillExecutor 与 SkillRecommender 实现了从技能注册、配置、执行到推荐的完整闭环。**更新** 新的技能推荐系统已从内部_yaml配置文件重构为外部context参考文档模式，技能信息迁移到data/context/目录下的markdown文档中，推荐逻辑由agent运行时直接读取这些文档完成。这种重构提高了系统的灵活性和可维护性，同时保持了接口的向后兼容性。通过阶段映射矩阵与跨阶段通用技能，系统在不同业务阶段提供一致的能力支撑；通过统计与安全扫描，保障执行质量与合规性。结合 ManagerAgent 的回退机制，系统在复杂场景下仍能稳定运行。

## 附录

### 技能配置管理
- 技能参数设置
  - 使用 update_config 动态更新技能配置，适用于运行时调整行为。
- 执行环境配置
  - 通过 required_permissions 与 security_scan 控制执行环境与安全策略。
- 安全控制
  - 依赖权限检查与安全扫描结果，必要时拒绝执行或降级处理。
- **更新** 配置格式选择
  - 新系统推荐使用 YAML 格式的 _registry.yaml 文件，提供更好的可读性。
  - 保持 registry.json 兼容性，确保平滑过渡。
  - **更新** 外部context文档管理：在 data/context/ 目录下维护技能推荐所需的 markdown 文档。

**章节来源**
- [skill_registry.py:376-383](file://backend/app/core/skill_registry.py#L376-L383)
- [skill_registry.py:56-63](file://backend/app/core/skill_registry.py#L56-L63)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)

### 技能扩展机制
- 自定义技能开发
  - 在 .agents/skills 下新增目录与 SKILL.md 描述文件，遵循技能元数据规范。
- 插件化架构
  - 注册表支持多种来源（builtin/local/github），便于插件化扩展。
- 动态加载
  - 执行器按技能名解析资源路径，支持本地与远程资源动态加载。
- **更新** 配置文件管理
  - 新增技能时，优先在 _registry.yaml 中添加配置条目。
  - 保持与现有 JSON 配置的兼容性。
  - **更新** 外部context文档集成：技能推荐信息通过 data/context/ 目录下的 markdown 文档管理。

**章节来源**
- [SKILL.md 示例](file://.agents/skills/gpt-taste/SKILL.md)
- [skill_registry.py:50-52](file://backend/app/core/skill_registry.py#L50-L52)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)

### 开发与集成指南
- 技能编写规范
  - 明确技能元数据（name/version/source/file_path/status/events/permissions/dependencies/config）。
  - 在 SKILL.md 中定义执行入口与参数说明。
- 测试方法
  - 使用执行器的 execute 方法进行单元测试，覆盖正常、超时、异常分支。
  - 基于执行记录与统计指标评估性能与稳定性。
- 部署流程
  - 将技能资产放入 .agents/skills 对应目录。
  - 通过注册表进行 register/install/enable，确保状态为 active。
  - **更新** 优先使用 _registry.yaml 进行配置管理，保持 registry.json 兼容性。
  - **更新** 在 data/context/ 目录下维护相应的技能推荐文档。
  - 重启后生效。

**章节来源**
- [后端api.md](file://后端api.md)
- [前后端api交互.md](file://前后端api交互.md)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)

### API 文档与使用案例
- API 参考
  - 注册表接口：register_skill、install_skill、uninstall_skill、enable_skill、disable_skill、update_config、get_by_id、list_skills、get_stage_matrix、get_cross_stage_skills。
  - 执行器接口：execute（技能名、参数、超时）。
  - **更新** 推荐器接口：recommend_by_event、recommend_by_stage、recommend_by_context（现已迁移到外部context文档）。
- 使用案例
  - **更新** 事件驱动：通过外部context文档提供技能推荐，快速响应不同类型的事件。
  - **更新** 阶段驱动：在特定业务阶段自动推荐适用技能，技能信息来自 data/context/ 目录下的 markdown 文档。
  - **更新** 上下文驱动：结合业务阶段与产品类型生成个性化推荐，推荐逻辑直接从外部文档读取。
- **更新** 配置管理案例
  - 新式配置：使用 YAML 格式在 _registry.yaml 中添加技能配置。
  - 兼容性迁移：逐步将现有 JSON 配置迁移到新的 YAML 格式。
  - **更新** 外部文档管理：在 data/context/ 目录下维护技能推荐所需的 markdown 文档。

**章节来源**
- [后端api.md](file://后端api.md)
- [skill_registry.py:688-708](file://backend/app/core/skill_registry.py#L688-L708)
- [skills/_registry.yaml](file://backend/data/skills/_registry.yaml)
- [skills/registry.json](file://backend/data/skills/registry.json)
- [shopify_reference.md:1-112](file://backend/data/context/shopify_reference.md#L1-L112)