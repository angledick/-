...
## 更新摘要
**所做更改**
- 新增系统事件总线增强，包含新增事件类型：risk:fraud_detected、feishu:message_received
- 新增Shopify连接同步作业跟踪，支持products、orders、inventory三种同步类型
- 新增AutoPullEngine启动日志和详细同步进度日志记录机制
- 更新同步日志系统重构，包含启动日志、开始同步日志、完成同步日志和错误日志
- 新增同步任务状态管理和实时日志查询接口
- 新增物流追踪号注册和管理功能
...
**更新重点**：本次更新重点关注系统事件总线增强、Shopify连接同步作业跟踪和AutoPullEngine日志系统重构，为平台的自动化数据同步提供完整的日志跟踪能力。
...
- 事件存储：合并系统事件和用户事件的操作链，提供统一的事件存储接口
...
### 事件存储系统
- 事件存储合并系统事件和用户事件的操作链，提供统一的事件存储接口
- 系统事件存储在event_chain/system_events目录下
- 用户事件存储在event_chain/action_chains目录下
- 支持按条件筛选系统事件，包括事件类型、来源、严重级别等

```mermaid
classDiagram
class EventStore {
+add_system_event(chain_id, event_type, source, description_nl, severity, payload, tags) str
+add_action_event(user_id, event_type, source, description_nl, severity, payload, tags) str
+get_system_events(chain_id) List[Dict]
+get_user_events(user_id) List[Dict]
+filter_system_events(chain_id, event_type, source, severity, max_count) List[Dict]
+get_all_chain_ids() List[str]
}
class EventRecord {
+event_id : string
+event_type : string
...
### 新增系统事件总线增强
- 新增risk:fraud_detected欺诈检测事件，支持按严重级别critical进行告警
- 新增feishu:message_received飞书消息接收事件，用于记录飞书平台的消息交互
- 事件总线支持按事件类型、严重级别、产品ID进行过滤查询
- 事件存储采用全局事件总线和产品级事件链双重存储机制

**章节来源**
- [backend/data/global/events/bus.json:21417-21448](file://backend/data/global/events/bus.json#L21417-L21448)
- [backend/app/core/event_bus.py:525-796](file://backend/app/core/event_bus.py#L525-L796)

### 新增Shopify连接同步作业跟踪
- 支持products（产品）、orders（订单）、inventory（库存）三种同步类型
- 每种同步类型都有独立的任务配置和执行逻辑
- 同步作业状态持久化到jobs.json文件，包含开始时间、结束时间、同步数量等信息
- 物流追踪号注册功能支持批量注册和去重管理

**章节来源**
- [backend/app/core/auto_pull_engine.py:250-383](file://backend/app/core/auto_pull_engine.py#L250-L383)
- [backend/data/sync/jobs.json:1-2354](file://backend/data/sync/jobs.json#L1-L2354)
- [backend/app/core/auto_pull_engine.py:519-534](file://backend/app/core/auto_pull_engine.py#L519-L534)

### 新增AutoPullEngine启动日志和详细同步进度日志
- 引擎启动时记录AutoPullEngine started日志，包含任务数量信息
- 同步开始时记录Starting sync: {provider}/{sync_type}日志
- 同步完成时记录Sync completed: {items_synced} items synced日志
- 错误情况下记录Sync failed: {error}日志，便于问题定位

**章节来源**
- [backend/app/core/auto_pull_engine.py:165-166](file://backend/app/core/auto_pull_engine.py#L165-L166)
- [backend/app/core/auto_pull_engine.py:204](file://backend/app/core/auto_pull_engine.py#L204)
- [backend/app/core/auto_pull_engine.py:218-220](file://backend/app/core/auto_pull_engine.py#L218-L220)
- [backend/app/core/auto_pull_engine.py:245](file://backend/app/core/auto_pull_engine.py#L245)

## 依赖关系分析
- AutoPullEngine依赖事件总线进行事件发布，同时管理本地的日志和任务状态
- 事件总线依赖事件存储进行系统事件链持久化，同时向通知引擎分发事件
- 事件存储依赖产品存储进行事件链持久化
...
## 结论
避风港平台的日志管理以AutoPullEngine为核心，结合事件总线、事件存储、通知引擎与风险预警，形成"事件产生—持久化—路由—可视化"的完整闭环。AutoPullEngine提供了完整的同步日志记录和事件发布机制，支持按任务ID精确查询和分析。通过严格的存储结构与查询接口，能够实现按严重级别、类别、产品ID与用户维度的高效检索与分析。建议在生产环境中配合日志轮转与归档策略，确保长期稳定运行。

## 附录
- AutoPullEngine事件类型：system:sync_completed和system:sync_failed两种系统事件
- 新增系统事件：risk:fraud_detected（欺诈检测）、feishu:message_received（飞书消息）
