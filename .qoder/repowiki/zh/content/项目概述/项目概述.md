...
- [backend/data/scheduler/tasks.yaml](file://backend/data/scheduler/tasks.yaml)
- [backend/data/scheduler/bindings.yaml](file://backend/data/scheduler/bindings.yaml)
- [后端变更路线图.md](file://后端变更路线图.md)
</cite>

## 更新摘要
**所做更改**
- 新增完整的项目文档文件，提供更全面的项目介绍和架构说明
- 调度系统从11个任务优化为3个核心任务，提升系统效率
- Shopify集成得到显著增强，包括OAuth流程改进和Webhook处理优化
- 通知系统进行全面改进，支持多渠道推送和更灵活的通知配置
...
- [项目文档.md:1-200](file://项目文档.md#L1-L200)
...
### 调度系统优化
**更新** 调度系统已从11个任务优化为3个核心任务，提升系统效率和可维护性

- 优化后的核心任务
  - 合规检查任务：集中处理产品合规预检、法规监控和风险评估
  - Shopify 集成任务：统一管理产品同步、订单处理和库存更新
  - 通知推送任务：标准化多渠道通知发送和用户提醒

- 调度配置
  - 任务绑定：通过 bindings.yaml 文件定义任务间的依赖关系
  - 执行策略：支持定时执行、事件触发和手动调度
  - 错误处理：内置重试机制和异常通知

**章节来源**
- [backend/app/core/scheduler.py:1-200](file://backend/app/core/scheduler.py#L1-L200)
- [backend/data/scheduler/tasks.yaml:1-150](file://backend/data/scheduler/tasks.yaml#L1-L150)
- [backend/data/scheduler/bindings.yaml:1-100](file://backend/data/scheduler/bindings.yaml#L1-L100)

### Shopify 集成增强
**更新** Shopify 集成得到显著增强，包括OAuth流程改进和Webhook处理优化

- OAuth 流程改进
  - 统一的OAuth管理器，支持多店铺连接和令牌刷新
  - 安全的凭据存储和加密机制
  - 自动化的授权状态检查和错误处理

- Webhook 处理优化
  - 实时事件监听和处理机制
  - 支持产品变更、订单创建、库存更新等多种事件类型
  - 事件去重和幂等性保证
...
**章节来源**
- [backend/app/api/shopify.py:1-300](file://backend/app/api/shopify.py#L1-L300)
- [backend/app/services/shopify_api.py:1-250](file://backend/app/services/shopify_api.py#L1-L250)

### 通知系统改进
**更新** 通知系统进行全面改进，支持多渠道推送和更灵活的通知配置
...
- [backend/app/api/notifications.py:1-150](file://backend/app/api/notifications.py#L1-L150)
...
- 常见问题定位
  ...
  - 调度任务异常：检查任务配置、绑定关系和执行日志。
  - Shopify 集成失败：验证OAuth凭据、Webhook配置和API限制。
- 环境与依赖
...
经过本次更新，系统在调度效率、Shopify集成和通知系统方面得到显著改进，为用户提供更加稳定和高效的合规服务体验。
